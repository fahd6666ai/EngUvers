...
export default function AdminExamsPage() {
  ...
  function load() {
    apiClient.get<AdminExam[]>('/admin/exams').then(setExams, (err) => {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <Card className="mb-8">
        <h2 className="mb-4 font-semibold">{t('newExam')}</h2>
          ...
          {exams.map((exam) => (
            <Link key={exam.id} href={`/admin/exams/${exam.id}`}>
              <Card className="flex items-center justify-between gap-3 transition hover:border-brand-500">
                <div>
                  <h3 className="font-medium">{exam.titleEn}</h3>
                  <p className="text-xs text-slate-400">
                    {t('questionCount', { count: exam._count.questions })} ·{' '}
                    {t('attemptCount', { count: exam._count.attempts })}
                  </p>
                </div>
